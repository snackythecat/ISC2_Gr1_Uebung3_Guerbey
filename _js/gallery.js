...
function storeNotes() {
  ...
  notes.addEventListener("blur", function () {
    let key = document.querySelector("img").src;
    console.log(key);
    let note = notes.innerHTML;
    if (notes.value !== "") {
...
function loadNotes(key) {
  ...
  const saveNotes = localStorage.getItem(key);
  if (localStorage.getItem(key) === null || localStorage.getItem(key) == "") {
    field.innerHTML = "Enter your notes here!";
  } else {
    field.innerHTML = saveNotes;
  }
}
